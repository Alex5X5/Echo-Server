...
public class Console9 extends JFrame{
...
//		Logging.buildLogMessage(mLvl, new ActionMessage("starting threads listening for main console"));
				...
				try {			
					while (true) {
						if (consoleInputI.available()!=0) {
							this.consoleLineHandler.sendMessage(this.readStdLine(consoleInputI));
						}
						if (quit) {
							return;
						}
				...
				try {
					while (true) {
						if (consoleInputE.available()!=0) {
							this.consoleLineHandler.sendMessage(this.readStdLine(consoleInputE));
						}
						if (quit) return;
					}
				} catch (Exception e) {
	...
	private void setupCommandField(Console9 c) {
...
//			Logging.buildLogMessage(new SimpleLogging.Logging.LoggingLevel("Console9"), new SimpleLogging.Logging.ActionMessage("buffering message"), new SimpleLogging.Logging.MessageParameter("message",s));
...
//		private GraphicsEnvironment graphicsEnvironment = GraphicsEnvironment.getLocalGraphicsEnvironment();
		...
		public ConsoleLine (int x, int y, Console9 c, int p) {
			super();
			this.pos = p; 
			this.setLayout(null);
...
//					if(pos==0)newT = commandScanner.next();
	...
	public interface CommandHandler{
		public void onCommandOutput(String command, Console9 console);
	}
...
//		}
		...
		@Override
		public void onCommandOutput(String command, Console9 c) {
		}
	}
