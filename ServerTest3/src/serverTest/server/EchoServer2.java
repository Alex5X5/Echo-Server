...
//			Logging.buildLogMessage(mLvl,new ActionMessage("added new Thread "),new MessageParameter("pos",i));
	...
	public static void main(String[] args) {
		@SuppressWarnings("unused")
		EchoServer2 sv = new EchoServer2();
	}
...
//		public boolean alive = true;
		...
		public EchoConnection(EchoServer2 es, int i) {
			try {serverSocket =	new ServerSocket(i);} catch (IOException e) {}
			Thread st = new Thread(
					()->{
						while(!serverSocket.isClosed()) {
							if(!mt.isAlive()) mt = new Thread(
									()->{
										LoggingLevel sLvl = new LoggingLevel("Connection thread "+(i-es.basePort+1));
											try {
